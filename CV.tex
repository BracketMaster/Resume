% !TEX TS-program = xelatex
% !TEX encoding = UTF-8 Unicode
% -*- coding: UTF-8; -*-
% vim: set fenc=utf-8

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% CV.tex
%% <https://github.com/zachscrivena/simple-resume-cv>
%% This is free and unencumbered software released into the
%% public domain; see <http://unlicense.org> for details.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% See "README.md" for instructions on compiling this document.

\documentclass[letterpaper,MMMyyyy,nonstopmode]{simpleresumecv}
\usepackage{fontspec}
\usepackage[colorlinks=true]{hyperref}
\setmainfont[BoldFont={Latin Modern Roman/B}]{CMU Concrete}
% Class options:
% a4paper, letterpaper, nonstopmode, draftmode
% MMMyyyy, ddMMMyyyy, MMMMyyyy, ddMMMMyyyy, yyyyMMdd, yyyyMM, yyyy

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% PREAMBLE.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% CV Info (to be customized).
\newcommand{\CVAuthor}{Yehowshua Immanuel}
\newcommand{\CVTitle}{Yehowshua Immanuel}
\newcommand{\CVNote}{CV compiled on {\today} for Acme Corporation}
\newcommand{\CVWebpage}{https://yehowshuaimmanuel.com/about}

% PDF settings and properties.
\hypersetup{
pdftitle={\CVTitle},
pdfauthor={\CVAuthor},
pdfsubject={\CVWebpage},
pdfcreator={luaLaTeX},
pdfproducer={},
pdfkeywords={},
unicode=true,
bookmarks=true,
bookmarksopen=true,
pdfstartview=FitH,
pdfpagelayout=OneColumn,
pdfpagemode=UseOutlines,
hidelinks,
breaklinks}

% Shorthand.
\newcommand{\Code}[1]{\mbox{\textbf{#1}}}
\newcommand{\CodeCommand}[1]{\mbox{\textbf{\textbackslash{#1}}}}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% ACTUAL DOCUMENT.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{document}

%%%%%%%%%%%%%%%
% TITLE BLOCK %
%%%%%%%%%%%%%%%

\Title{\CVAuthor}

\begin{SubTitle}
\href{yehowshuaimmanuel.com}
\par
\href{mailto:yimmanuel3@gatech.edu}
{yimmanuel3@gatech.edu}
\,\SubBulletSymbol\,
+1\,(678)\,216-6333
\,\SubBulletSymbol\,
\href{\CVWebpage}
{\url{\CVWebpage}}
\end{SubTitle}

\begin{Body}

%%%%%%%%%%%%%%%
%% INTERESTS %%
%%%%%%%%%%%%%%%
\Section
{Interests}
{Interests}
{PDF:Interests}
\BulletItem
Compilers, ISA Design, Hardware and Verification Language Tooling, Mathematics and Electromagnetics

\end
%%%%%%%%%%%%%%%
%% EDUCATION %%
%%%%%%%%%%%%%%%

\Section
{Education}
{Education}
{PDF:Education}

\Entry
\href{https://www.gatech.edu}
{\textbf{Georgia Institute of Technology | Atlanta, GA}}
\hfill
\DatestampYMD{2019}{01}{07} --
Present

\BulletItem
Masters in Computer Engineering

\BigGap
\Entry
\href{https://www.gatech.edu}
{\textbf{Georgia Institute of Technology | Atlanta, GA}}
\hfill
\DatestampYMD{2017}{05}{15} --
\DatestampYMD{2018}{12}{13} 

\BulletItem
Bachelor of Science in Computer Engineering
\hfill
GPA: 3.62 / 4.0

\BigGap
\Entry
\href{http://kennesaw.edu}
{\textbf{Kennesaw State University | Marietta, GA}}
\hfill
\DatestampYMD{2016}{06}{15} --
\DatestampYMD{2017}{05}{15}

\BulletItem
Transfer with 60 credit hours.
\hfill
GPA: 4.0 / 4.0


%%%%%%%%%%%%%%%%%%%%%%%%%
%% SKILLS %%
%%%%%%%%%%%%%%%%%%%%%%%%%

\Section
{Skills}
{Skills}
{PDF:Skills}

\Entry
\textbf{Computing}
\begin{Detail}
\BulletItem
\textbf{Software Languages}
\SubBulletItem
C(expert in C, proficient in rest), C++, Java, Python, Basic HTML, BASH(and most standard UNIX utils.)
\BulletItem
\textbf{Hardware Languages}
\SubBulletItem
VHDL, Verilog, Python Migen HDL, Some Bluespec
\BulletItem
\textbf{Software Tools}
\SubBulletItem
Docker, Git, \LaTeX, VIM, Regular Expressions, some Apache
\BulletItem
\textbf{Development Tools}
\SubBulletItem
GNU/BSD Make, GCC, Sub tools(ie: ar, nm)
\BulletItem
\textbf{APIs and Protocols}
\SubBulletItem
Ethernet, UART 
\BulletItem
\textbf{Hardware}
\SubBulletItem
Raspberry Pi, FPGAs, Arduino, Embedded Arm, PCB Design
\end{Detail}

\BigGap
\Entry
\textbf{Arts}
\begin{Detail}
\BulletItem
\textbf{Music}
\SubBulletItem
Piano, Bass, LMMS(Linux MultiMedia Studio)
\BulletItem
\textbf{Film and Animation}
\SubBulletItem
Blender, Synfig Studio, Inkscape
\BulletItem
\textbf{Languages}
\SubBulletItem
English, proficient in French
\end{Detail}

%%%%%%%%%%%%%%%%%%%%%%%%%
%% EXPERIENCE %%
%%%%%%%%%%%%%%%%%%%%%%%%%

\Section
{Experience}
{Experience}
{PDF:Experience}

\Entry
\href{http://synergy.ece.gatech.edu}
{\textbf{Synergy Lab}},
ECE Department, Georgia Tech
\begin{Detail}
\BulletItem
Master's Student,
\hfill
\DatestampYMD{2018}{05}{15} --
\DatestampYMD{2018}{08}{15}
\SubBulletItem
Implementing a reconfigurable ML hardware accelerator in an FPGA and accompanying compiler+driver stack
\end{Detail}

\Entry
\href{https://www.gtri.gatech.edu/laboratories/electronic-systems-laboratory}
{\textbf{Electronic Systems Warfare Lab}},
Georgia Tech Research Institute
\begin{Detail}
\BulletItem
Student Assistant
\hfill
Summer 2018 and 2019
\SubBulletItem
Verified the mathematics of various mechanical models of simulated agents 
in the Scrimmage simulation engine.
\SubBulletItem
Scripted and deployed Git version control commit rules
\SubBulletItem
Built BSD POSIX Libraries for VXWorks RTOS and wrote accompanying drivers.
\end{Detail}

\Entry
\href{http://retrotech.library.gatech.edu}
{\textbf{RetroTech}},
Georgia Tech
\begin{Detail}
\BulletItem
Student Assistant
\hfill
\DatestampYMD{2017}{08}{15} --
\DatestampYMD{2018}{05}{15}
\SubBulletItem
Diagnosed and restored legacy computers such as the Macintosh SE 1987. Deployed Verilog logic analyzer in FPGA to facilitate this process
\end{Detail}

\Entry
\href{https://www.gtri.gatech.edu/laboratories/sensors-and-electromagnetic-applications-laboratory}
{\textbf{Sensors and Electromagnetic Applications Laboratory}},
Georgia Tech Research Institute
\begin{Detail}
\BulletItem
Student Assistant
\hfill
\DatestampYMD{2017}{05}{15} --
\DatestampYMD{2017}{08}{15}
\SubBulletItem
Designed a printed circuit power board for an airborne radar. 
\end{Detail}

%%%%%%%%%%%%%%%%%%%%%%%%%
%% PROJECTS %%
%%%%%%%%%%%%%%%%%%%%%%%%%

\Section
{Projects}
{Projects}
{PDF:Projects}

\Entry
\href{https://github.com/ghdl/ghdl}
{\textbf{GHDL Maintainer}}
\hfill
\DatestampYMD{2020}{01} --
Present
\begin{Detail}
\BulletItem
One of the maintainers of the Free and Open Source(FOSS) VHDL simulator GHDL.
\end{Detail}
\Gap

\Entry
\href{https://libre-riscv.org}
{\textbf{LibreSOC Engineer}},
\hfill
\DatestampYMD{2019}{12} --
Present
\begin{Detail}
\BulletItem
I am a member of the LibreSOC organization currently funded by NLNet to create an
 open source RISCV capable complete SOC with a GPU that is open source to the bedrock.
\end{Detail}
\Gap

\Entry
\href{https://github.com/m-labs/nmigen}
{\textbf{nMigen Verilator Backend}},
\hfill
\DatestampYMD{2019}{12} --
Present
\begin{Detail}
\BulletItem
Currently adding Verilator simulator backend support to the nMigen Python HDL.
\end{Detail}
\Gap

%%%%%%%%%%%%%%%%%%%%%%%%%
%% LEADERSHIP %%
%%%%%%%%%%%%%%%%%%%%%%%%%

\Section
{Leadership}
{Leadership}
{PDF:Leadership}

\Entry
{\textbf{Chi Alpha  Worship Band}}
\hfill
\DatestampYMD{2018}{08}{15} --
Present
\begin{Detail}
\BulletItem
Played bass for worship team.
\end{Detail}
\Gap

\Entry
{\textbf{FutureTech Middle School Outreach}}
\hfill
\DatestampYMD{2018}{03}{15}
\begin{Detail}
\BulletItem
Lead FutureTech Exhibit at Georgia Tech to introduce middle schoolers to Computer Engineering
\end{Detail}
\Gap

\Entry
{\textbf{BCM Worship Band}}
\hfill
\DatestampYMD{2016}{08}{15} --
\DatestampYMD{2017}{05}{15}
\begin{Detail}
\BulletItem
Played piano for worship team.
\end{Detail}
\end{document}
